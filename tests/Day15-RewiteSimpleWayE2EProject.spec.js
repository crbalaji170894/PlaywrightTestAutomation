...
test.only("E2E Project1", async ({ page }) => 
{
    ...
    await page.locator('.card-body').filter({ hasText: productToCart }).getByRole("button", { name: " Add To Cart" }).click();

    //click carts after add to cart

    await page.getByRole("listitem").getByRole("button", { name: "Cart" }).click();

    await page.locator('.cart li').last().waitFor();

    // check the product text is visble in Carts page

    await expect(page.getByText(productToCart)).toBeVisible();
...
const countryLocator = page.getByPlaceholder("Select Country");
