// Login UI with before all method

// if we have 5 test cases

// before all will excute once berfore 5 test excution

// where once login done we collect all the broser level cookies, session token what even there in 
// application strogar part under network tab

//then keep this as json file

// then inject this json file which has all strogge credentials, cookies to broswer we perform each test globally
...
test.beforeAll( async ({browser})=>{
    ...
    const conntext= await browser.newContext();

    const page= await conntext.newPage();
    ...
    await page.waitForLoadState('load'); // dont forgot to add this line **('netowkidle')

    // after login succcesfull collect the storage data

    //method we have storagestate (browser level) use context.storage state
    await conntext.storageState({path:'state.json'});

    //create new context and inject the state.json file

    webContext2= await browser.newContext({storageState:'state.json'});

    // webContext2 has all the content about above login page

    // use this webContext2 in all the test cases
});

test("testcase2", async () => {


    // test method we d ont has page fixture hence  we are use ing new webcontext2 where we ncreate new page
